...
log="log_run_foldseek_aligntype2.txt"
...
# Define output dir for foldseek results within the subdirectory 
                output_dir="$subdirectory/foldseek_results_aligntype2"
...
# Before we run foldseek, let's check that the output file doesnt already exist (we may have already run foldseek in a previous sbatch job 
		    ...
		    foldseek easy-search "$input_file" AFproteome "$output_dir/$(basename "${input_file%.txt}.aln1.m8")" foldseek.tmp --alignment-type 2 --tmscore-threshold 0.0 -e 10 --format-output "query,target,mismatch,gapopen,pident,qstart,qend,qlen,tstart,tend,tlen,alnlen,evalue,qaln,taln,cigar,qseq,tseq"
