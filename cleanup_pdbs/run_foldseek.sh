...
for subdirectory in "$parent_directory"/*cp; do # loop through subdirectories ending in "cp"
    if [ -d "$subdirectory" ]; then  # Check if it's a directory
        ...
        for input_file in "$subdirectory"/*; do
...
# Define output dir for foldseek results within the subdirectory 
                output_dir="$subdirectory/foldseek_results"
...
# Before we run foldseek, let's check that the output file doesnt already exist (we may have already run foldseek in a previous sbatch job 
                if [ ! -f "$output_file" ]; then
                    ...
                    foldseek easy-search "$input_file" AFproteome "$output_dir/$(basename "${input_file%.txt}.aln1.m8")" foldseek.tmp --alignment-type 1 --tmscore-threshold 0.0 -e 10 --format-output "query,target,mismatch,gapopen,pident,qstart,qend,qlen,tstart,tend,tlen,alnlen,evalue,alntmscore,qtmscore,ttmscore,qaln,taln,cigar,qseq,tseq"        
                else
